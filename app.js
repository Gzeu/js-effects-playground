...
async function loadCryptoData() {
  ...
  container.innerHTML = `
    <div style="padding:2rem; text-align:center;">
      ...
      <p style="color:var(--text-sec); margin:1rem 0;">Mock Binance data loaded from mock-binance.csv</p>
...
function healthCheck() {
  ...
  logsOutput.textContent += '[INFO] K8s API Server: ✓ Healthy\n';
  logsOutput.textContent += '[INFO] Pods: 12/15 Running\n';
  logsOutput.textContent += '[INFO] KrakenD Gateway: ✓ Healthy (Latency: 45ms)\n';
  logsOutput.textContent += '[INFO] Prometheus: ✓ Scraping metrics\n';
  logsOutput.textContent += '[SUCCESS] All systems operational\n';
}

function parseLogs() {
  const logsOutput = document.getElementById('logs-output');
  if (!logsOutput) return;
  logsOutput.textContent = 'Parsing KrakenD logs...\n\n';
  logsOutput.textContent += '[2025-11-30 19:45:12] GET /api/v1/crypto - 200 OK (42ms)\n';
  logsOutput.textContent += '[2025-11-30 19:45:15] GET /api/v1/users - 200 OK (38ms)\n';
  logsOutput.textContent += '[2025-11-30 19:45:18] POST /api/v1/deploy - 201 Created (156ms)\n';
  logsOutput.textContent += '[2025-11-30 19:45:21] GET /health - 200 OK (5ms)\n';
}
...
function initMonaco() {
  ...
  fetch('snippets.json')
    .then(r => r.json())
    .then(data => { snippetsData = data; })
    .catch(() => console.log('Snippets not loaded'));
}
...
function runCode() {
  ...
  try {
    if (code.includes('<') && code.includes('>')) {
      ...
      doc.write(code);
      doc.close();
      consoleOutput.textContent += '✓ HTML rendered in preview\n';
    } else {
...
function generateZip() {
  ...
  deployFiles.innerHTML = `
    <h4>📦 Production ZIP Generated</h4>
    <p>Structure:</p>
    <pre style="text-align:left; color:var(--text-main);">
devforge-playground.zip
├── index.html
├── app.js
├── styles.css
├── manifest.json
...
└── README.md
    ...
    <button onclick="downloadAssets()">⬇️ Download ZIP</button>
  `;
}
...
`;
  ...
  deployFiles.innerHTML = `
    <h4>⚙️ GitHub Actions YAML</h4>
    <pre style="text-align:left; color:var(--text-main); background:#000; padding:1rem; border-radius:8px; overflow-x:auto;">${yaml}</pre>
    <p style="margin-top:1rem;">Save as <code>.github/workflows/deploy.yml</code></p>
  `;
}
...
function generateVercel() {
  ...
  deployFiles.innerHTML = `
    <h4>▲ Vercel Config</h4>
    <pre style="text-align:left; color:var(--text-main); background:#000; padding:1rem; border-radius:8px; overflow-x:auto;">${config}</pre>
    <p style="margin-top:1rem;">Save as <code>vercel.json</code> or deploy via CLI: <code>vercel deploy</code></p>
  `;
}

function downloadAssets() {
  alert('Download initiated! Check browser downloads.\n\nNote: In production, implement JSZip library for client-side ZIP generation.');
}
